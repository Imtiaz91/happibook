...
namespace Happibook.Repository
{
    public class UserRepository : AuditableRepository<ApplicationUser, string>, IUserRepository
    {
        ...
        public async Task<TotalResultDTO<ApplicationUser>> GetAllForAdmin(JsonApiRequest request, string search)
        {
            ...
            {
                result = await this.DefaultListQuery.Include(x => x.Roles).GenerateQuery(request).ToListAsync();
                total = await this.DefaultListQuery.Include(x => x.Roles).ToListAsync();
            }
            else
            {
                result = await this.DefaultListQuery.Include(x => x.Roles).Where(x => (x.Id.StartsWith(search) || x.FirstName.StartsWith(search) || x.LastName.StartsWith(search) || x.PhoneNumber.StartsWith(search) || x.UserName.StartsWith(search))).GenerateQuery(request).ToListAsync();
                total = await this.DefaultListQuery.Include(x => x.Roles).Where(x => (x.Id.StartsWith(search) || x.FirstName.StartsWith(search) || x.LastName.StartsWith(search) || x.PhoneNumber.StartsWith(search) || x.UserName.StartsWith(search))).ToListAsync();
            }
