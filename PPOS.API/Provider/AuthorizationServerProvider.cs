...
namespace Happibook.API.Provider
{
    public class AuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            ...
            if (!context.HasError)
            {
                ...
                identity.AddClaim(new Claim(ClaimTypes.Name, validCredentials.Email)); 
                ...
                var permissions = await this.userRolePermissionService.GetUserRolePermissions(userDTO.Roles.Select(x => x.Id).ToList());
