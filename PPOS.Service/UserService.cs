       ...
       // private IOTPRepository otpRepository;
            ...
            : base(unitOfWork, unitOfWork.UserRepository)
        {
          ...
          //  this.otpRepository = otpRepository;
            ...
            this.roleManager = new ApplicationRoleManager(new ApplicationRoleStore(this.requestInfo));
        }
        ...
        public async Task<TotalResultDTO<UserDTO>> GetAllUserUpdateAsync(JsonApiRequest request, string search)
        {
            ...
            if (role == UserRoles.Admin)
            {
                ...
                foreach (var user in result)
                {
                    ...
                    {
                        item.Name = roles.FirstOrDefault(x => x.Id == item.Id).Name;
                    }
                }
        ...
        {
            dtoObject.Status = dtoObject.Status == UserStatus.None ? UserStatus.Active : dtoObject.Status;
            ApplicationUser user = dtoObject.ConvertToEntity();

            user = await this.Repository.CreateAsync(user, dtoObject.Password);
        ...
        public override async Task<UserDTO> UpdateAsync(UserDTO dtoObject)
        {
            ...
            {
                this.exceptionHelper.ThrowAPIException(string.Format(Message.InvalidObject, dtoObject.Email));
            }
