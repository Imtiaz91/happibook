...
namespace Happibook.Common.Helper
{
    public class JSONHelper
    {
        ...
        public static object ConvertJSONToObject(string json)
        {
            ...
            {
                return JsonConvert.DeserializeObject(json, new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
            }
        ...
        public static dynamic GetDeserializedObejct(string result)
        {
            dynamic data = JObject.Parse(result.Replace("[", string.Empty).Replace("]", string.Empty));
            return data;
        }
    }
}
